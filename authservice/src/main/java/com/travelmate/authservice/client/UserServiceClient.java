...
@FeignClient(name = "user-service", configuration = FeignClientConfig.class, fallbackFactory = UserServiceFallbackFactory.class)
public interface UserServiceClient {
    @GetMapping("/api/users/email/{email}")
    UserInfoDTO getUserByEmail(@PathVariable("email") String email);

    @PostMapping("/api/users/{userId}/verify")
    boolean verifyUserCredentials(@PathVariable("userId") String userId, @RequestBody UserInfoDTO credentials);
